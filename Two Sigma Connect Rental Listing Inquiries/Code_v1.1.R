...
featureDictionary <- data.frame(Feature = tolower(unlist(train_Data$features))) %>%
     ...
     arrange(desc(Total)) %>% filter(Total >= 10)
...
#--------reshape data
result_xgb <- data.frame(train_Data$listing_id, matrix(result_xgb,ncol = 3,byrow = TRUE))
...
xgb.importance(names(input_dataset),model = model_xgb)
