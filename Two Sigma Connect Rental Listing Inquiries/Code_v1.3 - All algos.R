...
input_dataset %>% filter(price == 11000) %>% summarise(n=n())
...
#input_dataset <- input_dataset %>% filter(price != 11000) #-------79%
...
result_nb <- predict(model_naive
              ...
              ,newdata = subset(test_dataset,select = -c(listing_id,created))
              ,type = c("raw"))

result_nb <- getPredictedDataset(result_nb,"nb")


#table(result_nb$PredictedVal_nb, input_dataset$interest_level)
